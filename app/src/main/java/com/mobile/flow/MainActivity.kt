...
class MainActivity : AppCompatActivity() {
        ...
        if (result.resultCode == SettingsActivity.RESULT_TIMER_SETTINGS_CHANGED) {
            ...
            if (sharedPreferences.getBoolean("wereTimerSettingsModified", false)) {
                ...
                if (!isTimerRunning) {
                    ...
                    when (fragmentType) {
                        "focus" -> {
                            val fragment = TimerFragment.Companion.newInstance(1, totalSessions, false)
    ...
    private fun showShortBreakFragment() {
        ...
        supportFragmentManager.beginTransaction()
            .replace(R.id.frame_layout, ShortBreakFragment())
            .commit()
    }
    ...
    private fun showLongBreakFragment() {
        ...
        supportFragmentManager.beginTransaction()
            .replace(R.id.frame_layout, LongBreakFragment())
            .commit()
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (fragmentType != null && sessionInfo != null) {
            when (fragmentType) {
                "focus" -> {
                    ...
                    if (sessionParts.size == 2) {
                        ...
                        val fragment = TimerFragment.Companion.newInstance(currentSession, totalSessions, false)
                        supportFragmentManager.beginTransaction()
                            .replace(R.id.frame_layout, fragment)
                            .commit()
                    }
            ...
            TimerService.Companion.appOpened(this)
        } else {
            // Normal app start
            supportFragmentManager.beginTransaction()
                .replace(R.id.frame_layout, TimerFragment())
